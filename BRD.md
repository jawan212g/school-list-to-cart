...
| **D-7** | Classroom mode ships as a quantity multiplier, not a separate interface                        | The data model already carries an entity type and a student count, so supporting classroom quantities is a field rather than a feature. Before extraction, the parent must choose whether each classroom list states per-student quantities or whole-class totals. A dedicated bulk-buyer interface remains Phase 2.                                                                                                                                                                           |
...
- **FR-05** A child entry may be marked as a classroom group with a student count. On the section-selection screen, the parent must choose whether that classroom list states per-student quantities or whole-class totals. Per-student requirements are multiplied by the classroom count; whole-class totals are used exactly as written (D-7, BR-33).
